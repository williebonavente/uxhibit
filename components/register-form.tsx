  ...
  return (
    ...
    >
      ...
      <div className="scale-[0.95] origin-top">
        <div
          className="relative z-10 flex flex-col w-full max-w-sm sm:max-w-md md:max-w-lg xl:max-w-2xl
               p-6 sm:p-8 md:p-10 lg:p-12 bg-[#1E1E1E]/40 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20"
        >
          ...
          <p className="text-sm sm:text-base md:text-lg text-center mb-8 text-white dark:text-[#F5F5F5]/70">
            Create your account with UXhibit.
          </p>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
                ...
                <div className="flex-1">
                  <FormField
                    ...
                    render={({ field }) => {
                      ...
                      const todayStr = format(today, "yyyy-MM-dd"); // used to cap future dates
                      return (
                        <FormItem className="h-full">
                          ...
                          <FormControl>
                            <div className="flex gap-2">
                              {/* Wrapper to show custom grey placeholder and hide native icon */}
                              <div className="relative w-full">
                                <Input
                                  ...
                                  max={todayStr} // (1) disallow selecting future (after today)
                                  min="1900-01-01" // optional lower bound
                                  className={`no-native-date appearance-none w-full h-11 text-sm border-white/20
                             text-[#1A1A1A] dark:text-white bg-white dark:bg-[#1A1A1A]
                             ${!field.value ? "text-opacity-70" : ""}`}
                                  style={{
                                    // (3) hide native picker icon (Chrome / Edge / Safari)
                                    // For Firefox, appearance:none already helps.
                                    // Extra padding so overlay placeholder fits.
                                    paddingRight: "0.75rem",
                                  }}
                                />
                                {/* (2) Greyish creative placeholder shown only when empty */}
                                {!field.value && (
                                  <span
                                    className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2
                               text-xs tracking-wide text-gray-400/70 dark:text-gray-500/60"
                                  >
                                    MM/DD/YYYY
                                  </span>
                                )}
                              ...
                              <Popover>
                                <PopoverTrigger asChild>
                                  <Button
                                    type="button"
                                    variant="outline"
                                    className="h-11 px-3 text-sm border-white/20 text-[#1A1A1A] dark:text-white bg-white dark:bg-[#1A1A1A]"
                                ...
                                >
                                  <Calendar
                                    ...
                                    // (1) disable future dates (after today)
                                    disabled={[
                                      {
                                        from: new Date(
                                          today.getFullYear(),
                                    ...
                                    // Optional range limit (keep historical reasonable)
                                    hidden={{
              ...
              <div className="mt-8 w-full flex justify-center cursor-pointer">
                {siteKey ? (
                  <ReCAPTCHA
                    ref={captchaRef}
                    sitekey={siteKey}
