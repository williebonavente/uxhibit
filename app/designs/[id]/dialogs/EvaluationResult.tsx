...
const EvaluationResult: React.FC<EvaluationResultProps> = ({
  ...
  return (
    <>
      ...
      {evalResult.ai?.resources && evalResult.ai.resources.length > 0 && (
        <div className="p-5 rounded-2xl bg-[#0D9488]/10 dark:bg-[#2DD4BF]/10 mb-6 shadow-md">
          ...
          <ul className="flex flex-col gap-4">
            {evalResult.ai.resources.slice(0, 5).map((resource: any, i: number) => {
      ...
      {/* Fallback grace */}
      {(!evalResult.ai ||
        !Array.isArray(evalResult.ai.resources) ||
        evalResult.ai.resources.length === 0) && (
        <div
          className="p-4 rounded-md bg-amber-50 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200"
          role="alert"
        >
